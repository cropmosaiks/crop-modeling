...
one_sensor_date <- "2022-11-09"
two_sensor_date <- "2022-11-09"
...
# sensor_1      <- 'sentinel-2-l2a_2-3-4-8_15_False_True_True'
# sensor_2      <- 'landsat-8-c2-l2_1-2-3-4-5-6-7_20_True_True_True_True'
# summary_logo  <- paste0('summary-pred_logo-cv_fn-1_',sensor_1,'_fn-2_',sensor_2,'_he-True.csv')
summary_logo  <- 'summary_logo_2_sensor_results_2022-11-11.csv'
...
one_sensor_results <- here::here("data", "results", one_sensor_fn) %>% 
  ...
  mutate(avg_res = (kfold_val_R2 + kfold_demean_R2) / 2,
         avg_all = (kfold_val_R2 + kfold_demean_R2 + logo_val_R2) / 3)
...
two_sensor_results <- here::here("data", "results", two_sensor_fn) %>%
  ...
  mutate(
    ...
    weighted_avg = case_when(
      ...
      T ~ 'mixed'),
    avg_res = (kfold_val_R2 + kfold_demean_R2) / 2,
    avg_all = (kfold_val_R2 + kfold_demean_R2 + logo_val_R2) / 3) 
...
logo_predictions <- here::here('data', 'results', summary_logo) %>% 
  readr::read_csv() %>% 
  mutate(split = factor(split, levels = c('train', 'test')))
...
hist_plot <- function(data, x) {
  x     <- enquo(x)
  x_lab <- expand_field(x)
  if(deparse(substitute(data))=='two_sensor_results'){n_sensors = 2}else{n_sensors=1}
  if(n_sensors == 1){clr<-'dodgerblue'}else{clr<-'darkorchid'}
  if(n_sensors == 1){n_bins<-30}else{n_bins<-60}
  if(n_sensors == 1){lms<-limits}else{lms<-limits_two}
  ...
  ggplot(data = data) +
    ...
    labs(x = x_lab) +
    scale_x_continuous(limits = lms, expand = expansion)
}
...
pred_plot <- function(.data, x, y, label = NULL, x_lims = c(0, .82),
  ...
  p <- ggplot() +
    ...
    labs(color = NULL, x = 'Observed', y = 'Predicted') +
...
As a secondary test, we train an additional model (for each unique set of inputs) with leave one group out CV where year is the grouping variable. This means the training data is split into $n$ groups, where $n$ is the number of years in the data, with each group containing $1/n^{th}$ of the available data. Each iteration of the model is trained using $n-1$ groups as training data, and is validated on the remaining group. This process is repeated $n$ times, using a different group for validation on each iteration. The performance measures are calculated by taking the predicted and observed values of the validation data in each of the $n$ iterations. Our primary metric is the coefficient of determination calculated on the observed and predicted values of the validation data. 

![Figure 2. Diagram representing leave one group out cross validation data splitting strategy](../../figures/logo_cv_split.png)

We use LOGO CV as a secondary test for it ability to evaluate how well these models are able to predict over time... More on why later..

**Note**: Currently we are not splitting into train and test sets for LOGO CV. This is something that should be decided and finalized. It is important to note that we cannot split the data into train and test and do hot encoding (or maybe we can but its all 0 for a district not included? not sure actually). 
...
```{r}
hist_plot(one_sensor_results, logo_val_R2)
```
...
```{r}
hist_plot(two_sensor_results, logo_val_R2)
```
...
```{r}
hist_plot(one_sensor_results, logo_demean_R2)
```
...
```{r}
hist_plot(two_sensor_results, logo_demean_R2)
```
...
```{r}
hist_plot(anomaly_results, kfold_val_R2)
```

#### LOGO

```{r}
hist_plot(anomaly_results, logo_val_R2)
```
...
1) **Spectral bandpass**: Landsat 8 bands 1, 2, 3, 4, 5, 6, and 7 (coastal/aerosol, blue, green, red, near infrared, short-wave infrared 16, short-wave infrared 22)    

1) **Spatial resolution**: 20,000 0.01 by 0.01 degree grid cells, representing top 10% of cropland in each district.

1) **Number of features**: 1,000 for each month (6,000 total).

1) **Limiting month range**: Landsat 8: true, the months were limited to the dry season (from April to September).    
    
1) **Crop mask**: Landsat 8: true, only points with cropland are retained.  
...
1) **Available year range**: 2013 to 2021, the overlap of available satellite data and crop yield data. 

**Notes**: 

1) Need to discuss how to select the "best model" (see email). 
...
```{r }
ggplot(data = high_res_predictions, aes(x = prediction)) +
  geom_histogram(bins = 30, fill = 'dodgerblue', color = 'black') 
```
...
The top performing model over time used ridge regression with leave one group out cross validation, repeated for 17 regularization parameters, with the top performing regularization parameter used for model fitting. 
...
1) **Spectral bandpass**:  
    ...
    + S2 bands: B, G, R, NIR   
    + LS8 bands: B, G, R, NIR, SWIR16, SWIR22, AOT  

1) **Spatial resolution**: 

    + S2: 15,000 0.01 by 0.01 degree grid cells, sampled form an evenly spaced grid.    
    + LS8: 20,000 0.01 by 0.01 degree grid cells, representing top 10% of cropland in each district  
...
1) **Crop mask**:  
    ...
    + LS8: true, by defualt with the targeted method of selecting points described above  
    
1) **Weighted average**: 

    + Ss2: true, a weighted average was used in summarizing the features to the district level.
    + LS8: true, a weighted average was used in summarizing the features to the district level.  
...
```{r}
train_pred <- filter(logo_predictions, split == "train")

val_n     <- length(logo_predictions$logo_cv_prediction)
val_R2    <- r2_general(logo_predictions$log_yield, logo_predictions$logo_cv_prediction)
demean_R2 <- r2_general(logo_predictions$demean_yield, logo_predictions$logo_demean_prediction)

val_label    <- plot_label(val_R2, val_n)
demean_label <- plot_label(demean_R2,   val_n)
```
...
##### LOGO Validation Predictions

```{r warning = FALSE, fig.width=5, fig.height=5}
pred_plot(
  logo_predictions,
  x = log_yield, 
  y = logo_cv_prediction, 
  label = val_label
)
```
...
```{r warning = FALSE, fig.width=5, fig.height=5}
min_x = min(logo_predictions$demean_yield)
max_x = max(logo_predictions$demean_yield)

min_y = min(logo_predictions$logo_demean_prediction)
max_y = max(logo_predictions$logo_demean_prediction)

pred_plot(
  .data = logo_predictions, 
  x = demean_yield, 
  y = logo_demean_prediction, 
  label = demean_label,
