...
# high_res_fn   <- "high-res-pred_k-fold-cv_best-k-fold-2-sensor-params_he-True.feather"
# summary_fn    <- 'summary-pred_k-fold-cv_best-k-fold-2-sensor-params_he-True.csv'
...
pred_plot <- function(.data, y, label){
  y <- enquo(y)
  p <- ggplot() +
    geom_point(data = .data,
               aes(x = log_yield, y = !!y, color = as.factor(year))) +
    geom_abline() +
    scale_color_viridis_d() +
    labs(color = NULL, x = 'Observed', y = 'Predicted') +
    geom_text(data = NULL, aes(x = 0.05, y = .7), label = label) +
...
1) **Spatial resolution**: 20,000 0.01 by 0.01 degree grid cells, sampled form an evenly spaced grid.

1) **Number of features**: 1,000 for each satellite, for each month (18,000 total).
...
val_R2_label <- latex2exp::TeX("$R^2$ = \\r2", user_defined = list("\\r2"=val_R2))

train_R2 <- r2_general(train_pred$log_yield, train_pred$prediction)
train_R2_label <- latex2exp::TeX("$R^2$ = \\r2", user_defined = list("\\r2"=train_R2))

test_R2 <- r2_general(test_pred$log_yield, test_pred$prediction)
test_R2_label <- latex2exp::TeX("$R^2$ = \\r2", user_defined = list("\\r2"=test_R2))
```
...
```{r warning = FALSE}
pred_plot(train_pred, y = kfold_cv_predictions, val_R2_label)
```

#### Train

```{r warning = FALSE}
pred_plot(train_pred, prediction, train_R2_label)
```

#### Test

```{r warning = FALSE}
pred_plot(test_pred, prediction, test_R2_label)
```
...
**Note**: Need to add in the cropland map.
