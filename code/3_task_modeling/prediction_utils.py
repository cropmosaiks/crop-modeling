...
def custom_ridge(X,y, lam, intercept=True, 
    ...
    ----------
        ...
        static_lam_val :numeric or list-like: 
            Second hyperparameter option for a subset of the X columns. 
            If set to 0 then, some Xs will not be penalized
            If a list-like array is given, it should be same length as static_lam_idxs.
            A different static hyperparameter can be passed to each of the indices provided.
    ...
    if static_lam_val is not None:
        ...
        if hasattr(static_lam_val, '__iter__'):
            assert len(static_lam_val) == len(static_lam_idxs)
        else:
            static_lam_val = np.full(len(static_lam_idxs), static_lam_val)
                
        for i, idx in enumerate(static_lam_idxs):
            eye[idx,idx] = static_lam_val[i]

    model = linalg.solve(XtX + eye, 
    Xty, sym_pos=True)
    ...
    for lx, lambdan in enumerate(lambdas):
        ...
        if svd_solve:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                model, intercept_term = custom_ridge(None,None,lam=lambdan,intercept=intercept, # No need to input X and Y
    ...
    **kwargs_solve,
    ...
    if fit_model_after_tuning:
        if solve_function is not ridge_regression:
            ...
            rets["prediction_model_intercept"]) = custom_ridge(X,y, lam=best_lambda, 
                                                               intercept=intercept,
...
def time_group_trend_demean(df, var_cols, group_col="State", time_col = "Year", polynomial_order=1, verbose=False, debug=False):
...
def mfe(df, var_cols, group_cols, time_group=None, weights_col=None, n_iter=4, verbose=False, time_trend_polynomial=1, debug=False):
    """
    ...
    if any([order > 1 for order in time_trend_polynomial]):
            
        warnings.warn( ("Reprojecting with polynomial time trends is in beta mode. "
                        "Additional work is needed to demonstrate that outputs from "
                        "this procedure are equivelant to outputs from R/Stata canned packages. "
                        " \n"
                        "Results reported using polynomial time trends should be considered preliminary."))
    ...
    for j in range(n_iter):
        ...
        if time_group is not None:
            for k, item in enumerate(time_group):
                if j > 0 and time_trend_polynomial[k] > 1:
                    warnings.warn( ("Polynomial time trends will only be residualized once. "
                                    "This overrides the n_iter argument"))
                    continue
                ...
                df = time_group_trend_demean(df,var_cols, group_col=cat,time_col=cont, 
                                             polynomial_order=time_trend_polynomial[k], verbose=verbose, debug=debug)
