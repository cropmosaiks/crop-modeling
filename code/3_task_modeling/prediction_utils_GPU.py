...
def custom_ridge(X,y, lam, intercept=True, 
    ...
    eye = lam*xp.eye(XtX.shape[1], dtype=np.float64)
    ...
    model = linalg.solve(XtX + eye, Xty,**linalg_solve_kwargs,) 
    model = asarray(XtX + eye, Xty)
...
def ridge_regression(
    ...
    """
    # send to CPU if necessary
    # X_train = asnumpy(X_train)
    # y_train = asnumpy(y_train)
