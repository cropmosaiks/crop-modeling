...
#   read_csv(show_col_types = FALSE) %>% 
#   mutate(
#     sat_combo = paste0(satellite_1,' with ',satellite_2),
#     month_combo = case_when(
#       month_range_1 == '4-9' & month_range_2 == '4-9' ~ '4-9',
#       month_range_1 == '1-12' & month_range_2 == '1-12' ~ '1-12',
#       T ~ 'mixed')) %>% 
#   filter((points_1 != 20 & points_2 != 20) | (crop_mask_1 == T & crop_mask_2 == T))
...
expand_fld <- function(fld) {
  switch(
    rlang::as_name(fld),
    'kfold_val_R2'    = 'Validation Score (R$^2$)',
    'kfold_demean_R2' = 'De-meaned Score (R$^2$)',
    'month_combo'     = 'Month\nRange',
    'month_range'     = 'Month\nRange',
    'hot_encode'      = 'One Hot Encoding',
    'crop_combo'      = 'Crop Mask',
    'crop_mask'       = 'Crop Mask',
    'wa_combo'        = 'Weighted Average',
    'weighted_avg'    = 'Weighted Average',
    rlang::as_name(fld)
  ) %>% TeX()
}

custom_plot <- function(data, x, y, clr, p_type = "scatter", n_sensor = 1) {
  x     <- enquo(x)
  y     <- enquo(y)
  clr   <- enquo(clr)
  x_lab <- expand_fld(x)
...
The first statistic we evaluate is the coefficient of determination (R<sup>2</sup>) calculated as the average R<sup>2</sup> of the 5 validation sets. 
