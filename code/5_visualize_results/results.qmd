...
expand_fld <- function(fld) {
  ...
  switch(
    fld,  
    'val_R2'       = 'Validation Score (R2)',
    'demean_R2'    = 'De-meaned Score (R2)',
    'month_range'  = 'Month\nRange',
...
All models use ridge regression on data summarized to the district level (both yield and features) and split into a training set (80%) and a test set (20%). The model is trained using 5-fold cross validation. This means the training data is split into five folds, each with 20% of the available training data and the model is trained using 4 of the folds as training data, and is validated on the remaining fold. This process is repeated 5 times, using a different fold for each iteration. The performance measures are calculated by taking the average of the 5 iterations.    

![](../../figures/data_split.png)
