...
```{r}
he <- here::here("data","random_features", "summary", "landsat-c2-l2_bands-r-g-b-nir-swir16-swir22_ZMB_20k-points_1024-features_yr-2009-2021_mn-4-9_lm-True_cm-True_wa-True_he-True_summary.feather") %>% 
  arrow::read_feather()

he_f <- here::here("data","random_features", "summary", "landsat-c2-l2_bands-r-g-b-nir-swir16-swir22_ZMB_20k-points_1024-features_yr-2009-2021_mn-4-9_lm-True_cm-True_wa-True_he-False_summary.feather") %>% 
  arrow::read_feather()
...
#     year = dplyr::case_when(month %in% 10:12 ~ year + 1, T ~ as.numeric(year))) %>%
...
```{r}
ndvi <- readr::read_csv(here::here('data', 'climate', 'modis_ndvi.csv'))
pre_tmp <- readr::read_csv(here::here('data', 'climate', 'cru_pre_tmp.csv'))
...
X_train <- as.matrix(subset(train, select = -c(yield_mt, year
                                               , district
                                               )))
y_train <- train$yield_mt

X_test <- as.matrix(subset(test, select = -c(yield_mt, year
                                             , district
                                             )))
