...
```{r}
crop_yield <- here::here('data', 'crop_yield', 'cfs_maize_districts_zambia_2009_2022.csv') %>% 
  readr::read_csv() %>% 
...
ndvi <- here::here('data', 'climate', 'NDVI', 'NDVI.csv') %>% 
  ...
  dplyr::mutate(ndvi = mean(ndvi, na.rm = TRUE)) %>% 
...
climate <- temp %>% 
  ...
  dplyr::ungroup() %>% 
  dplyr::mutate(across(.cols = 3:38, .fns = , ~(scale(.) %>% as.vector))) %>% 
  dplyr::relocate(yield_mt, .after = year)

round(colMeans(climate[, 4:39]), 10) 
...
climate <- climate %>% 
  mutate(yield_mt = log10(1+yield_mt))

df <- climate 
# df <- dplyr::select(climate, 1:15)
# df <- dplyr::select(climate, 1:3, 16:27)
# df <- dplyr::select(climate, 1:3, 28:39)
